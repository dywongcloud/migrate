...
- The build/demo/bench scripts in `scripts/` run *inside* a Lima VM, not on the macOS host: they use Linux-only tooling (`mkfs.ext4`, `mount -o loop`, `ip`, `podman`, `make`, `/dev/kvm`). Lima mounts `/Users` into the VM at the same path, so the hardcoded `/Users/dylanwong/daedal` paths resolve identically there. Enter with `limactl shell daedal-kvm` (or `daedal-pvm` for the PVM path) before running them. Each such script sources `scripts/lib/guard.sh` and calls `require_linux` up front, so running one on the macOS host fails fast with an actionable message instead of a cryptic `command not found`. Portable scripts (`check-ascii.sh`, `fetch-guest-images.sh`, `patch-pvm-msr.py`) do not carry the guard.
